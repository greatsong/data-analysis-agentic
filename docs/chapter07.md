...
| **핵심 활동** | 검토 라운드(짝의 분석 점검하기) + 개선 라운드(피드백 반영하기) |
...
```mermaid
flowchart LR
    A["도입<br>5분"] --> B["활동 1: 검토 라운드<br>20분"]
    B --> C["활동 2: 개선 라운드<br>15분"]
    C --> D["정리: 성찰<br>10분"]
```
...
<div class="tl-label">검토 라운드: 짝의 분석 점검하기</div>
<span class="tl-time">20분</span>
</div>
<div class="tl-item">
<div class="tl-dot">3</div>
<div class="tl-label">개선 라운드: 피드백 반영하기</div>
...
!!! warning "교사 주의: 건설적 피드백 유도"
    비판적 검토가 감정적으로 흐를 수 있습니다.
...
??? tip "시간이 부족할 때"
    - **검토 라운드 단축**: 2회전 → 1회전 (10분 절약)
    - **성찰 간소화**: 손들기 투표만 실시 (5분 절약)
    - **핵심 유지**: 검토 라운드는 절대 줄이지 말고, 개선 라운드에서 시간 조절
...
flowchart TD
    ...
    B --> C[검토 라운드 1: 이동 → 점검 → 기록]
    C --> D[검토 라운드 2: 이동 → 점검 → 기록]
    D --> E[자리 복귀: 피드백 읽기]
    E --> F[개선: 수용/반론/보류 분류]
    F --> G[1:1 대화: 쟁점 논의]
    G --> H[성찰: 개선 계획 작성]
