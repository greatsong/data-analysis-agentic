...
1~3차시에서 배운 모든 것을 단계적 도움(스캐폴딩) 없이 독립 수행합니다. | 2시간
</div>
...
| **핵심 활동** | 독립 프로젝트: 주제 선택 → 질문 설계 → 분석 지침서(Soul Document) → AI 분석 → 검증 → 발표 |
...
!!! abstract "이번에는 아무것도 안 줍니다"
    ...
    **4차시에는 아무것도 주지 않습니다.**
    ...
    이것이 바로 **에이전틱 러닝(학생 주도 AI 활용 학습)**의 최종 목표입니다.
...
```mermaid
flowchart LR
    A["도입<br>5분"] --> B["활동 1: 주제+질문<br>10분"]
    B --> C["활동 2: 지침서+분석<br>15분"]
    C --> D["활동 3: 검증+준비<br>10분"]
    D --> E["발표<br>10분"]
```
...
```mermaid
flowchart TD
    A["주제 선택<br>'내가 진짜 궁금한 것'"] --> B["질문 설계<br>3단 변환 적용"]
    B --> C["분석 지침서 작성<br>AI 지시서 완성"]
    C --> D["AI 분석 실행<br>결과 수신"]
    D --> E["결과 검증<br>5단계 체크"]
    E --> F{"검증 통과?"}
    F -->|No| G["지침서 수정 → 재분석"]
    G --> D
    F -->|Yes| H["3분 발표<br>질문→방법→결과→한계→의의"]
```
...
```mermaid
flowchart TD
    CP1["체크포인트 1<br>10분: 질문 완성"] --> CP2["체크포인트 2<br>25분: 지침서 + 분석 완료"]
    CP2 --> CP3["체크포인트 3<br>35분: 검증 + 발표 준비"]
    CP3 --> P["발표<br>40~50분"]
